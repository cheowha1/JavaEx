...
//		arrayCopySystem();
	...
	private static void multiDimArrayEx() {
		...
		// 인덱스 볌위 : table[0] ~ table[table.length -1]
		System.out.println("table[0].length: " + table[0].length);
		// 인덱스 범위: table[0][0]~ table[table[0]table[0].length -1]
		
		// table 배열 내부의 모든 요소의 합을 구하자
		int sum = 0;
		for (int row = 0; row < table.length; ++row) { // 행 로프
			for (int col = 0; col < table[row].length; ++col) { // 열 로프
